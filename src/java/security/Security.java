...
public final class Security {
    ...
    private static void initialize() {
        ...
        if ("true".equalsIgnoreCase(props.getProperty
            ...
            // CustomJSSE provider hook
            // Enable CustomJSSE provider from standard location:
            // <java.home>/lib/ext/customjsse.jar
            boolean customJSSEEnabled = "true".equals(VM.getSavedProperty("org.customjsse.provider"));


            if (((openJSSEEnabled ? 1 : 0) + (legacy8uJSSEEnabled ? 1: 0) + (customJSSEEnabled ? 1 : 0)) > 1)  {
                throw new ProviderException(
                        "Unable to use OpenJSSE, Legacy8uJSSE and CustomJSSE providers simultaneously");
                ...
                if (setJSSEProvider("Legacy8uJSSE", "legacy8ujsse", null, props))
                    loadedProps = true;
            } else if (customJSSEEnabled) {
                if (setJSSEProvider("CustomJSSE", "customjsse", null, props))
                    loadedProps = true;
            }
        }
